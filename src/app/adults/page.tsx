...
export default function AdultsPage() {
  const programs = [
    ...
    },
    {
      ...
      subtitle: "일본어 그림책 모임",
      desc: "일본어를 몰라도, 책이 없어도 괜찮습니다. 그림책을 좋아하는 마음 하나면 충분해요. 번역본과는 다른, 원서만이 전해주는 매력 속으로.",
      meta: ["매달 마지막 주 금요일 · 13:00–15:00", "강사: 김연정 선생님 (일본어 강사, 그림책테라피스트)"],
  ...
  const workshop = {
    ...
    subtitle: "창작 모임",
    desc: "『마음빨래』 남개미 작가와 함께, 공모전·투고·더미데이를 목표로 꾸준히 작업하는 스터디. 혼자 떼기 어렵던 첫 발을 같이 내딛습니다.",
  ...
  return (
    <main style={{ fontFamily: "Pretendard,-apple-system,sans-serif", background: "#fff", minHeight: "100vh", letterSpacing: "-0.012em" }}>
      ...
      <div style={{ padding: "72px 40px 64px", background: "#0A0A0A", color: "white", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ fontSize: "44px", fontWeight: 800, letterSpacing: "-0.04em", lineHeight: 1.1, marginBottom: "20px", maxWidth: "560px" }}>읽고, 쓰고,<br />나로 돌아오는 시간.</div>
        <div style={{ fontSize: "13px", color: "#888", lineHeight: 1.9, maxWidth: "460px" }}>필사, 그림책, 영어 원서, 일본어 그림책, 창작 워크숍까지. 어른을 위한 깍지모모의 독서 모임이에요.</div>
      </div>

      {/* 카드 그리드 */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderLeft: "2px solid #0a0a0a", borderTop: "2px solid #0a0a0a" }}>
        {programs.map((p) => (
        ...
        <div style={{ ...cardStyle, gridColumn: "span 2" }} className="adult-card">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 2rem", alignItems: "flex-end" }}>
